十五、函数的语法：
函数在多数情况下叫做方法
主函数main方法是一个指挥者，负责统筹大局（把不同的功能交给内部不同的方法去实现，主函数负责调用），子函数是具体的执行者
15.1、方法是一段能独立完成功能的代码块。写一次方法可以多次调用，提高代码复用率
15.2、方法的语法：

<public> <static> 返回值类型[如果没有返回值则在这填写void] 方法名 ([数据类型 参数名...]) {  // 方法体  // return 结果（如果是void就不需要return）}
public    static   void                                  main  (String[] args){    int age =10; 
										   System.out.println("第一次打印："+ age);
										   }
 如果指定了数据类型，那么必须要有return（如果有返回值的类型，就必须要有一个return被执行才不会报错），是void的话就不需要return
 方法名要见名致意，按驼峰形式命名
15.3、方法的定义和调用：
...
//        调用函数
//        compare(a,b)  他一定会返回一个int类型的数 因此需要一个int类型的变量接收这个结果
        int result =
	;
        System.out.println("比较大的数字" + result);
        
    }
//            整数比较 这个是子函数，用来完成具体功能
...
15.4、方法参数的内存结构：
当一个class类加载到内存中的时候会划分出一片空间，如果这个类里面有如上所示的一个方法（compaerNum）,这个方法也会加载到内存，有一个地方就是专门存储
这种方法的，叫做静态方法区。当静态方法区中的方法被调用的时候：就会给方法里面的变量创造出对应的变量类型的空间，此时的空间里面是空的。当执行到
 compareNum(a,b) 括号里面的代码的时候，会把10，和30，立刻传入到静态方法中对应的那两个空的变量里面
 
 所以main方法中变量的地址和compareNum方法中变量的地址在内存中是不一样的
 
 我们管主函数调用方法时传递的变量叫实参，管方法中的参数叫做形参，我们调用方法时是通过值传递的方式把实参的值传递给形参
 
 java语言中只有值传递
 
 15.5、方法的重载：
 重载（overload）：在同一个类中有两个或两个以上方法名字一样但是参数不同（参数类型和个数不同），和返回值无关
十四、条件语句
14.1、switch：
char grade = 'a'  
...
11.2、播放mp3时的整个过程：
mp3文件在硬盘中---》（双击酷狗播放器）就会把酷狗的应用程序加载到内存当中比如占500m内存。酷狗播放器应用程序一定会有一个自己的存储空间
（应用程序把自己的500m分割了一部分空间用来存储）用来放置需要播放的歌曲（也就是用来运行mp3程序）当把哪首歌加载到存储空间的时候就会运行
那个mp3文件也就是执行mp3那个文件的代码。当一首歌放完之后就把该mp3文件从存储空间中移除，然后加载下一首到存储空间。
存储歌曲的那个空间就是内存中存储可变动文件的空间，在程序运行的时候存储空间里面的内容发生改变

在物理空间上：
变量：在程序运行的过程中用来存储可以变化的数据的内存空间叫做变量。首先，变量是一个内存中划分出来的一份空间，然后，该空间里面的数据可以在程序
运行时发生改变

十二、在java程序上如何体现变量：
数据类型：分为两大类：基本数据类型和引用数据类型
数据类型就是规定该变量在内存中所属哪种空间
12.1、基本数据类型：一共八种：整数类型型（包函四类），小数类型（包函两类），字符串类型、布尔类型
在电脑程序中：0和1叫做位，八个这样的位叫做一个字节
（1）、字节型（byte）：是最小的整数类型，占用内存中的存储空间大小是一个字节，该字节的最大值是127（八个位中的第一位是符号位，不能算作数值），
	最小值是-128（当我们存储一个数据明确知道他是小于100的就用byte来存储，因为一个很小的整数去占用一个比较大的空间是浪费）
	当我们定义了一个byte类型的变量的时候也就是约束了该变量是整数，并且值的大小位于-128到127之间。如果不满足这两个条件就会报错。

（2）、短型（short）：占两个字节的存储空间（也就是16个位）。必须是整数，数值大小范围在2的-15次方到2的15次方减一。
（3）、int ：占四个字节。是最常用的整数类型
（4）、long：占八个字节。
（5）、float：占四个字节。（小数的数据存储方式和整数不一样，不用纠结）float比int和long表示的数据范围大
（6）、double：占八个字节
（7）、字符类型（char）：占两个字节。如：'z'但是在char类型下写成'zx'是不行的，因为这是字符串，不是字符
（8）、布尔类型（boolean）：
12.2、变量的定义：
语法：数据类型 变量名字 = 初始值;    如：int age =10; 变量的定义必须要有初始值。如果没有会在编译的时候报错（注意，定义类的属性的时候可以不初始化
但是在方法中定义的变量必须要有初始值）。


变量定义的位置：只能定义在方法或语句块中。需要记住一定要定义在方法中就行（语句块很少使用）。如：
public static void main(String[] args){
        int age =10;  // 这个语句的含义是我在运行的时候会在内存中开辟一个一个字节的空间。这个空间的名字叫age，这个空间中存的是整数10
        System.out.println(age);  // 这句话的意思是我可以根据age这个名字拿到对应空间里面的内容，并打印输出到控制台
    }
在main方法中定义变量age变量。
在物理上的过程就是：当运行java Demo的时候，会立刻在内存中生成一个jvm，在jvm空间里面执行class中的代码，执行的过程是在jvm中开辟了一个一个字节大小
的空间，名字叫age。
如果再添加代码：
public static void main(String[] args){
        int age =10;
        System.out.println("第一次打印："+ age); // 注意字符串和变量之间连接需要用 + 号来连接，否则会报错
        age = 20;
        System.out.println("第二次打印："+ age);
    }
此时表面已经定义的age空间里面的内容可以在程序运行的时候改变为20.这也就是变量的定义：在程序运行的过程中用来存储可以变化的数据的内存空间叫做变量

12.3、变量的作用范围
对变量的操作只有两种：存值和取值
变量的作用范围：变量只能在当前的大括号和当前大括号的字括号内使用

12.4、数据类型转换：
（1）、自动转换
变量在做计算的时候可能会出现溢出的情况（byte最大值127 当两个byte类型的变量相加的时候出现了129 就是溢出），java会采用自动类型转换的方式来处理这种
情况。
规律：byte、short、char其中任意几个类型在做运算的时候计算结果都会自动提升类型到int
	byte a = 10;
   	int b = a + 1;
        System.out.println(b);

	因为a是一个变量，计算机在识别他的时候不回去判断里面值的大小，而且变量就意味着里面的值会发生改变，所以b的值可能会超出byte的范围，因此当定义
	b的类型为byte的时候会报错，要定义成int类型.当定义byte类型里面是常量相加小于127的时候可以正常运算，和大于127会报错
	c常量在做运算的时候只要不超出定义类型的范围就不会报错，也不会发生类型自动提升
char在参与运算的时候每一个字符背后都对应着一个ASCII码，在运算的时候是按其背后的ASCII来进行计算的
int与其他类型的变量相加的时候不会类型提升，会返回一个损失精度的值（int一旦达到最大值就会损失精度），不能自动提升成long类型
任何整数类型和long类型进行计算都会转换成long类型

小数类型：
float和任何类型的数据（除double）计算结果都会提升为float
double和任何类型的数据计算结果都会提升为double
（2）、强制转换
一般用于大空间的数据类型向小空间的数据类型去转换
语法：（目标数据类型）变量
int i =100;
short s = (short)i;
byte b = (byte)i;
强制转换有可能损失精度，因为要转换的值可能会超过目标类型的最大值

小数类型转换成整数类型会舍弃小数位（不会四舍五入，直接舍弃小数位）

整型也可以转换成char类型，转换后的结果是ASCII码对应的内容

十三、运算符
13.1、算术运算符：
int f = a / b ;是取整（a=10,b=3则f的结果是3）因为f是int类型，所以是取整
a % b 是取余

13.2、当后加加和后减减和其他代码在同一行的时候，先使用++和--之前的值,如果不在同一行就会得到++或--之后的值
int a = 10 ;
 System.out.println(a++); // 输出结果是10
 
int a = 10; 
a++;
System.out.println(a); // 输出结果是11
前加加和前减减在与其他代码同一行的时候，使用++之后的值
int a = 10 ;
 System.out.println(++a); // 输出结果是11
